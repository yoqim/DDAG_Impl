...
class IWPA(nn.Module):
    ...
    def forward(self, x, feat, t=None, part=0):
        ...
        refined_part_feat = refined_part_feat.view(b, self.inter_channels, part) # B, C//r, T, Part         # [64, 2048, 3]

        gate = F.softmax(self.gate, dim=-1)
        weight_part_feat = torch.matmul(refined_part_feat, gate)                # [64, 2048]     

        x = F.adaptive_avg_pool2d(x, (1, 1))
        # weight_part_feat = weight_part_feat + x.view(x.size(0), x.size(1))

        weight_part_feat += feat
        feat = self.bottleneck(weight_part_feat)                                # [64, 2048] 
