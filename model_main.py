...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, adj, modal=0):
        ...
        x_pool = x_pool.view(x_pool.size(0), x_pool.size(1))            # x^o : [64, 2048]
        feat = self.bottleneck(x_pool)          

        if self.lpa:
            feat_att = self.wpa(x, feat, 1, self.part)                  # x^* : [64, 2048] 

        if self.training:
            # cross-modality graph attention
            x_g = F.dropout(x_pool, self.dropout, training=self.training)
            x_g = torch.cat([att(x_g, adj) for att in self.attentions], dim=1)          # x_g : [batch_size, nhead*low_dim]
            ...
            return self.l2norm(feat), self.l2norm(feat_att), self.classifier(feat)